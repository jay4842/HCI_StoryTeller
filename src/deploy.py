...
from src.gui.window import WindowGUI
from src.net.vgg import vgg16
from data.imagenet.imagenet_classes import class_names
import src.text.rnn_test as rnn_test # text generator
...
# - we will use two graphs and two sessio
...
def deploy(args):
    ...
    with tf.variable_scope('vgg'):
        ...
        vgg = vgg16(imgs, 'model_saves/vgg16/vgg16_weights.npz', vgg_sess)
    
    rnn = rnn_test.Tester(args, rnn_sess)
    ...
    #class_type = classify()
    class_type = classify(vgg, vgg_sess)
    print(class_type)
    #input('->')
    # now generate the rnn output
    story_output = rnn.run(args, rnn_sess, display=False)
    story_output = post(story_output, name_list, class_type)
    print(story_output)
